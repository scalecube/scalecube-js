...
import {isValidService} from './helpers/serviceValidations';
...
class Builder {
  ...
  services( servicesList: any[] ) {
    servicesList.map(( o ) => {
      ...
      if ( isValidService(o) ) {
        const service = new ServicesConfig(o);
        this.add(service);
      } else {
        console.error(Error('service must have meta property'));
      }
    });
    return this;
  }
