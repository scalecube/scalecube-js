...
Scenario: Successfully create Microservice using htmlServiceDefinition, htmlService & ASYNC_MODEL_TYPES
	Given 	htmlService with htmlServiceDefinition
...
Scenario: Test creating microservice from function constructor
	Given 	htmlService with htmlServiceDefinition
	And 		MethodRegistry
	When 		Microservice calls registerService
	And 		MethodRegistry return an error
	Then 		htmlServiceProxy is NOT created
	And 		relevant log message will be written 
